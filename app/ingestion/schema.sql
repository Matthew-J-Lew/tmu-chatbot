...
-- Used for common lookups and to keep source identity stable
-- 1. URL is treated as the canonical identity for a source document
-- 2. content_sha1 is still indexed for change detection and analytics (but is NOT a unique key)
DROP INDEX IF EXISTS ux_sources_content_sha1;
DROP INDEX IF EXISTS idx_sources_url;
CREATE UNIQUE INDEX IF NOT EXISTS ux_sources_url ON sources (url);
CREATE INDEX IF NOT EXISTS idx_sources_content_sha1 ON sources (content_sha1);


-- =============================================================
-- CRAWLING / URL DISCOVERY TABLES
-- -------------------------------------------------------------
-- These tables power a production crawl pipeline:
--  - crawl_profiles: scope + policy rules (seeds, allowed prefixes, deny patterns)
--  - crawl_targets: the approved URL queue ingestion reads from
--  - crawl_runs / ingest_runs: lightweight observability for scheduled runs
-- =============================================================
...
CREATE TABLE IF NOT EXISTS crawl_profiles (
  ...
  deny_path_regex      JSONB       NOT NULL DEFAULT '[]'::jsonb,
  strip_query          BOOLEAN     NOT NULL DEFAULT TRUE,
  include_pdfs         BOOLEAN     NOT NULL DEFAULT TRUE,
...
CREATE TABLE IF NOT EXISTS crawl_runs (
  id              BIGSERIAL PRIMARY KEY,
  profile_id      BIGINT      NOT NULL REFERENCES crawl_profiles(id) ON DELETE CASCADE,
...
CREATE TABLE IF NOT EXISTS crawl_targets (
  ...
  created_at       TIMESTAMPTZ NOT NULL DEFAULT NOW(),
  updated_at       TIMESTAMPTZ NOT NULL DEFAULT NOW(),
  CONSTRAINT ux_crawl_targets_profile_url UNIQUE (profile_id, normalized_url),
  CONSTRAINT chk_crawl_targets_status CHECK (status IN (
    'discovered', 'approved', 'blocked', 'queued', 'crawled', 'ingested', 'failed'
  ))
);

CREATE INDEX IF NOT EXISTS idx_crawl_targets_profile_status
  ON crawl_targets (profile_id, status);
CREATE INDEX IF NOT EXISTS idx_crawl_targets_priority
  ON crawl_targets (profile_id, priority DESC);
CREATE INDEX IF NOT EXISTS idx_crawl_targets_next_crawl
  ON crawl_targets (profile_id, next_crawl_at);
