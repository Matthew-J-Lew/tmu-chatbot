...
def _summary_text_for_container(container: Tag, context_heading: str) -> str:
    ...
    # Capture nearby intro text between the heading and the accordion wrapper.
...
def _extract_rich_text_blocks(panel_body: Tag) -> List[str]:
    texts: List[str] = []
    # Prefer the primary text column from the accordion panel.
    for rich in panel_body.select(".c1 .resText .res-text, .c1 .resText.parbase.section .res-text"):
        txt = _clean_text(_text(rich))
        if txt:
            texts.append(txt)
    if not texts:
        # Fallback: any rich text not clearly inside the secondary/testimonial column.
        for rich in panel_body.select(".resText .res-text, .resText.parbase.section .res-text"):
            ...
            if txt:
                texts.append(txt)
    # Attach infographic text to the owning program, but never let it be its own section.
    for info in panel_body.select(".resInfographic .textContainer"):
        ...
        if len(info_words) <= 8 and re.fullmatch(r"[\d$,+–\- ]+", info_text.splitlines()[0]):
            # Ultra-short stat blurbs are low-value noise.
            continue
        texts.append(f"At a glance: {info_text}")
    # Optional: include the testimonial quote, but only as supporting context.
...
def extract_html_document(html: str, base_url: str) -> HtmlDocument:
    ...
    for block in blocks:
        ...
        if not txt:
            continue
        # Avoid pure button/link-grid/footer leftovers.
        if len(txt.split()) < 4:
            continue
